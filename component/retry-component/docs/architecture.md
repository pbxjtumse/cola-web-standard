...
- `retry-core`：实现同步有限重试，仅复用 `foundation-id` 的通用字符串 ID 能力，不依赖 Spring、Micrometer、消息客户端或数据库。
- `retry-config`：完成 Spring Boot 配置、事件和指标适配。
- `retry-demo`：只做用法演示，不承载核心能力。

公共 API 的职责分包见 [包结构设计](package-layout.md)。
...
- `RetrySleeper`：负责同步等待，生产默认将 `Duration` 转换为 `Thread.sleep(millis, nanos)`。
- `StringIdGenerator`：由 `foundation-id` 提供逻辑执行 ID，默认使用 UUID v7。
