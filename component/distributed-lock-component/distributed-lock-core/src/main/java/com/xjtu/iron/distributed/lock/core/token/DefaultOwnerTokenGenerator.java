 ...
 * 默认 ownerToken 生成器。
 *
 * <p>生成结果包含节点标识、进程标识、线程名、时间戳和 UUID。这样做的目标不是让 token 可预测，
...
public final class DefaultOwnerTokenGenerator implements OwnerTokenGenerator {
    ...
    public DefaultOwnerTokenGenerator() {
        this(defaultNodeId(), defaultProcessId());
    }

    public DefaultOwnerTokenGenerator(String nodeId, String processId) {
        this.nodeId = sanitize(nodeId == null ? "unknown-node" : nodeId);
        this.processId = sanitize(processId == null ? "unknown-process" : processId);
    }

    @Override
    public String generate(String lockName, String lockKey) {
