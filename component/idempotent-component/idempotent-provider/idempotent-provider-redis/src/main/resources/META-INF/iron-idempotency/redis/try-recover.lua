...
-- 8 expectedOwner, 9 expectedVersion, 10 shardKey, 11 scanBucket
local key = KEYS[1]
...
if h('shard_key') ~= shardKey or h('scan_bucket') ~= scanBucket then return snapshot(7, '') end
