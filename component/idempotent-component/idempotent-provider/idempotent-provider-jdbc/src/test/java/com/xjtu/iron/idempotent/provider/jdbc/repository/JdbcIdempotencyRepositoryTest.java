...
class JdbcIdempotencyRepositoryTest {
    ...
    void expiredProcessingMustNotExtendSlidingWindowOnNormalDuplicate() {
        ...
        IdempotencyAcquireRequest firstRequest = new IdempotencyAcquireRequest(
                "window", "sliding", "hash", "merchant:1", "A",
                IdempotencyMode.WINDOWED,
                Duration.ofSeconds(1),
                Duration.ofMinutes(5),
                IdempotencyWindowPolicy.SLIDING_ON_ACCESS,
        ...
        IdempotencyAcquireRequest duplicateAfterProcessingExpired =
                new IdempotencyAcquireRequest(
                        "window", "sliding", "hash", "merchant:1", "B",
                        IdempotencyMode.WINDOWED,
                        Duration.ofSeconds(1),
                        Duration.ofMinutes(5),
                        IdempotencyWindowPolicy.SLIDING_ON_ACCESS,
        ...
        IdempotencyAcquireResult duplicate =
                repository.tryAcquire(duplicateAfterProcessingExpired);

        assertThat(duplicate.getStatus())
                .isEqualTo(IdempotencyAcquireStatus.PROCESSING_EXPIRED);
        assertThat(duplicate.getRecord().getWindowExpireAt())
                .isEqualTo(originalWindowExpireAt);
    }
